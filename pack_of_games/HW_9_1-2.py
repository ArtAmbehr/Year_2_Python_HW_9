...
def generate_coef_to_csv():
    with open("./9_HW/files/generator_hw.csv", 'w', newline="", encoding='utf-8') as f: 
...
def decorator_for_save_solution(func: Callable):
    @wraps(func)
    def wrapper(*args):
        file_solution = "./9_HW/files/solution_hw.json"
        file_coef = "././9_HW/files/generator_hw.csv"
        params = []
        result = func(*args)
        with (
...
def decorator_for_solution(func: Callable):
    @wraps(func)
    def wrapper():
        with open("./9_HW/files/generator_hw.csv", 'r', newline="", encoding='utf-8') as f:
